  ...
  useEffect(() => {
    async function loadTheme() {
      try {
        ...
        const raw = json.primaryColor as string; // e.g. '0xfffd0d00'
        console.log('raw color:', raw);
        // strip '0xff' prefix for RGB
        const cssHex = '#' + raw.substring(raw.length - 6);
        setPrimaryColor(cssHex);
      } catch {
        // fallback remains
      }
    }
  ...
  return (
    <main className="min-h-screen bg-[#FAF3EA] flex flex-col items-center px-4 sm:px-6 lg:px-12 py-6 space-y-6">
      ...
      <header className="w-full max-w-6xl grid grid-cols-3 items-center px-4">
        ...
        {/* right slot: points icon and menu */}
        <div className="flex items-center justify-end space-x-4">
          ...
          >
            <Star className="h-5 w-5" color={primaryColor}/>
          </button>
          <Menu options={options} buttonColor={primaryColor} />
        </div>
      </header>
      ...
      <section className="w-full max-w-6xl flex flex-col-reverse lg:flex-row items-center bg-[#F2EAE2] rounded-2xl overflow-hidden shadow-md">
        ...
        <div className="w-full lg:w-1/2 p-4 sm:p-6 lg:p-8 space-y-4 sm:space-y-6">
          ...
          <Button fullWidth color={primaryColor} className="text-sm sm:text-base py-2">
            ORDER NOW
          </Button>
        </div>
        {/* Hero Image */}
      ...
      {[{
         icon: <Mail className="h-10 w-10" color={primaryColor} />,
         title: 'Contact Us',
         ...
         btn: (
           <Button
             ...
             onClick={() => window.location.href = 'mailto:info@balibu.com'}
           >
             Contact
           </Button>
         ),
       },{
          icon: <BookOpen className="h-10 w-10" color={primaryColor} />,
          title: 'Our Story',
          text: 'Learn more about us',
          btn: <Button variant="outline" color={primaryColor} className="w-full text-sm py-2" onClick={() => setActiveModal('about')}>Read More</Button>,
        }].map((card, i) => (
          <Card key={i} bgColor="#F2EAE2" className="px-4 sm:px-6 py-6 flex flex-col items-center justify-center text-center space-y-3">
            {card.icon}
