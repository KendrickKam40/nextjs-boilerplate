...
export default function HomePage() {
  ...
  useEffect(() => {
    async function loadTheme() {
      try {
        ...
        if (!res.ok) return;
        const { primaryColor: raw } = await res.json() as { primaryColor: string };
        // strip '0xff' prefix for RGB
        setPrimaryColor('#' + raw.substring(raw.length - 6));
      } catch {
        // keep default
      }
    }
  ...
  // When redeeming, post a message into your hidden iframe
        ...
        <section className="w-full max-w-6xl grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 sm:gap-6">
          ...
          {[
            ...
            <Card
              key={i}
              bgColor="#F2EAE2"
              className="px-4 sm:px-6 py-6 flex flex-col items-center justify-center text-center space-y-3"
            >
              {card.icon}
